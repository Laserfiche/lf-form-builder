...
export class PostMessageHelper<Messages extends MessageMap> {
  ...
  private discoveryRetryHandle: ReturnType<typeof setInterval> | null = null;
  ...
  constructor(
    ...
    this.beginPeerDiscoveryProbe();
  }

  private beginPeerDiscoveryProbe(): void {
    if (!this.peerDiscoveryEnabled || this.isPeerDiscovered) {
      return;
    }

    // Avoid overlapping probe loops from repeated whenPeerDiscovered() calls.
    if (this.discoveryRetryHandle) {
      return;
    }
    ...
    let attempts = 1;
    this.discoveryRetryHandle = setInterval(() => {
      if (this.isPeerDiscovered || attempts >= maxAttempts) {
        if (this.discoveryRetryHandle) {
          clearInterval(this.discoveryRetryHandle);
          this.discoveryRetryHandle = null;
        }
        return;
      }
  ...
  private sendPeerDiscoveryMessage(phase: PeerDiscoveryPhase): number {
    ...
    for (const targetWindow of this.getSendTargets()) {
      try {
        ...
        // Ignore cross-origin target mismatches from unrelated sibling frames.
      }
    }
  ...
  public whenPeerDiscovered(): Promise<void> {
    // Retry discovery when callers start waiting. This covers cases where
    // the peer iframe is injected after helper construction.
    this.beginPeerDiscoveryProbe();
