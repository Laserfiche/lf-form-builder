...
export const registerStarHandler = async (
  ...
  if (voteOptions?.rowNumberFieldId) {
    ...
    // rowToStarMap[fieldFor] = {
    //   count: 0,
    //   maxStarCount: voteOptions?.maxStars ?? 5,
    // };
    // return LFForm.changeFieldSettings(starField, {
    //   textAbove: makeStarHtml(
    //     0,
    //     `field-for-${starFieldId}-undefined`,
    //     voteOptions?.maxStars
    //   ),
    // });
  }
};
